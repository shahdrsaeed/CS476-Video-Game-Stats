...
import { Shield, User, Users, ClipboardList, LayoutDashboard, LogOut } from 'lucide-react';

const Navbar = ({ role = 'player' }) => {
  const navigate = useNavigate();
  const location = useLocation();

  const links = [
    { label: 'OVERVIEW',      path: '/general',       icon: <LayoutDashboard size={14} /> },
    { label: 'MY PROFILE',    path: '/player',        icon: <User size={14} /> },
  ...
  const visibleLinks = links.filter(l => !l.coachOnly || role === 'coach');
